...
export function hasNitro(user: User): boolean {
    if (!user) throw new Error("Utilia: No user provided!");

    if (user.bot) return false;

    if (user.avatar && user.avatar.startsWith("a_")) return true;

    if (user.flags && (user.flags.has("PARTNERED_SERVER_OWNER") || user.flags.has("DISCORD_EMPLOYEE"))) return true;
